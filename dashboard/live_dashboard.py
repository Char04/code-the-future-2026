...
def apply_offset():
    global home_alt, stable_start_time, current_offset
    try:
        current_offset = float(offset_var.get())
...
warn_font = font.Font(family="Helvetica", size=18, weight="bold")
...
tk.Label(right_frame, text="ALTITUDE TAPE (REL)", fg="#d2dae2", bg='#1e272e', font=label_font).pack(pady=(20, 5))
...
lbl_alt_rel = tk.Label(right_frame, text="+0.00m", fg="#0fbcf9", bg='#1e272e', font=value_font)
lbl_alt_rel.pack(pady=(5, 15))

tk.Label(right_frame, text="LIMITA ALERTA (m)", fg="#ff5e57", bg='#1e272e', font=label_font).pack(pady=(0, 0))
max_alt_var = tk.StringVar(value="520.0")
tk.Entry(right_frame, textvariable=max_alt_var, font=label_font, width=10, bg="#2f3640", fg="white", justify="center").pack()

tk.Label(right_frame, text="SIMULARE ALTITUDINE (m)", fg="#0be881", bg='#1e272e', font=label_font).pack(pady=(15, 0))
offset_var = tk.StringVar(value="0.0")
tk.Entry(right_frame, textvariable=offset_var, font=label_font, width=10, bg="#2f3640", fg="white", justify="center").pack()
tk.Button(right_frame, text="APLICA SIMULARE", command=apply_offset, font=label_font, bg="#485460", fg="white").pack(pady=5)
...
warn_text = canvas.create_text(20, canvas_size - 20, text="ALERTA ALTITUDINE!", fill="#ff5e57", font=warn_font, anchor="sw", state="hidden")
...
def update_gui():
    ...
    try:
        if rel_alt >= float(max_alt_var.get()):
            canvas.itemconfig(warn_text, state="normal")
        else:
            canvas.itemconfig(warn_text, state="hidden")
    except ValueError: 
        pass
